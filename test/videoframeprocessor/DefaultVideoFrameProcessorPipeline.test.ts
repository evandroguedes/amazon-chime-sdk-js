  ...
  describe('setInputMediaStream', () => {
    ...
    it('can start the pipeline with valid stream and stop with null', async () => {
      ...
      const startCallback = new Promise(resolve => {
        pipeObserver.processingDidStart.callsFake(resolve);
      });

      const stopCallback = new Promise(resolve => {
        pipeObserver.processingDidStop.callsFake(resolve);
      });
      await pipe.setInputMediaStream(mockVideoStream);
      await startCallback;
    ...
    it('can fail to start pipeline and fire callback if buffers are destroyed', async () => {
      ...
      const failToStartCallback = new Promise(resolve => {
        pipeObserver.processingDidFailToStart.callsFake(resolve);
      });
  ...
  describe('setter processors', () => {
    ...
    it('can set slow processor and fires processingLatencyTooHigh', async () => {
      ...
      const latencyCallback = new Promise(resolve => {
        pipeObserver.processingLatencyTooHigh.callsFake(resolve);
      });

      const startCallback = new Promise(resolve => {
        pipeObserver2.processingDidStart.callsFake(resolve);
      });
